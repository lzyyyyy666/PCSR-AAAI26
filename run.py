...
def run():
    ...
    parser.add_argument("--noise_file",
                        default="",
                        help="noise_file")
    parser.add_argument("--noise_ratio", default=0.6, type=float, help="Noisy ratio")
    ...
    parser.add_argument("--warmup_epoch", default=5, type=int, help="Epochs of warm up stage.")
    parser.add_argument("--warmup_epoch_2", default=25, type=int, help="Epochs of training with clean data only.")
    parser.add_argument("--warmup_epoch_3", default=40, type=int, help="Epochs of training with clean+noisy data only.")
    parser.add_argument(
    ...
    parser.add_argument("--model_path",
                        default="",
                        help="The path to the loaded models")
    parser.add_argument("--po_dir", default="",
                        help="The path to PO data for resuming training")
    parser.add_argument("--resume", action="store_true", help="Resume training")

    # parser.add_argument("--resume", type=bool, default=True, help="Resume training (True/False)")
    parser.add_argument(
    ...
    if opt.multiprocessing_distributed and ngpus_per_node == 0:
        raise RuntimeError("No GPU devices found for multiprocessing distributed training.")

    total_world_size = ngpus_per_node * opt.world_size if opt.multiprocessing_distributed else opt.world_size
    if opt.distributed:
        if opt.batch_size < total_world_size:
            raise ValueError(
                "batch_size must be >= total number of processes. "
                f"Got batch_size={opt.batch_size}, total_world_size={total_world_size}."
            )
        if opt.batch_size % total_world_size != 0:
            print(
                f"[Warning] batch_size={opt.batch_size} is not divisible by total_world_size={total_world_size}. "
                "Using floor division for per-process batch size."
            )
        opt.batch_size = max(1, opt.batch_size // total_world_size)

    if opt.multiprocessing_distributed:
        # now, opt.world_size means num of total processes in all nodes
        opt.world_size = total_world_size
        print('run: ', opt.gpu)
    ...
    should_run_test = (not opt.distributed) or opt.multiprocessing_distributed or (opt.rank == 0)
    if should_run_test:
        print("\n*-------- Testing --------*")
        if opt.data_name == "coco_precomp":
            ...
            evalrank(os.path.join(opt.output_dir, "checkpoint_best_test.pth.tar"), split="test")
            evalrank(os.path.join(opt.output_dir, "checkpoint_best_validation.pth.tar"), split="test")
            evalrank(os.path.join(opt.output_dir, "checkpoint_latest_validation.pth.tar"), split="test")
