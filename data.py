...
class PrecompDataset(data.Dataset):
    ...
    def __init__(
        ...
        if data_split == "train":
            ...
            if self.mode == "labeled":
                ...
                split_idx = np.nonzero(pred == 1)[0]  # Extract 1D indices directly
                ...
                # split_idx = np.where((pred != 0) & (pred != 1))[0]  # false
                split_idx = (pred == 0).nonzero()[0]
                ...
                # split_idx = (pred == 0).nonzero()[0]  # false
                split_idx = np.where((pred != 0) & (pred != 1))[0]
    ...
    def __getitem__(self, index):
        ...
        if self.data_split == "train":
            if self.mode == "labeled" or self.mode == "hard":
                return (
                    image,
                    text,
                    index,
...
def get_loader(
    ...
    def build_loader(dataset, shuffle):
        sampler = DistributedSampler(dataset, shuffle=shuffle) if (is_distributed and shuffle) else None
    ...
    if data_split == "warmup":
        ...
        data_loader = build_loader(dset, shuffle=True)
        ...
        labeled_trainloader = build_loader(labeled_dataset, shuffle=True)

        hard_dataset = PrecompDataset(
            name,
            captions,
            images,
            "train",
        ...
        hard_trainloader = build_loader(hard_dataset, shuffle=True)
        ...
        unlabeled_trainloader = build_loader(unlabeled_dataset, shuffle=True)

        return labeled_trainloader, unlabeled_trainloader, hard_trainloader

    elif data_split == "dev":
        dset = PrecompDataset(name, captions, images, data_split)
        data_loader = build_loader(dset, shuffle=False)
