...
class ContrastiveLoss(nn.Module):
    ...
    def forward(
        ...
        if mode == "predict":
            ...
            # Select clean samples only
            return all_loss.sum()
        elif mode == "warmup":
            return cost_s_mean.sum() + cost_im_mean.sum()
        elif mode == "lb_train" or mode == "ulb_train" or mode == "hard":
            if hard_negative:
...
class GCELoss(nn.Module):
    """
    Compute non-contrastive loss with GCE instead of CE
    """
    ...
    def forward(self, logits, targets, use_hard_labels=True, reduction='mean'):
        if use_hard_labels:
            probs = F.softmax(logits, dim=1)
            en_loss_cap = entropy(torch.mean(targets, 0), input_as_probabilities=True)
            en_loss_img = entropy(probs, input_as_probabilities=True)
            ...
            _, max_idx = torch.max(targets, dim=-1)
            p_true = probs[torch.arange(probs.size(0)), max_idx]

            if self.q == 1.0:
                gce_loss = 1.0 - p_true
            else:
                gce_loss = (1.0 - p_true.pow(self.q)) / self.q

            if reduction == 'mean':
                gce_loss = gce_loss.mean()
            elif reduction == 'sum':
                gce_loss = gce_loss.sum()

            return gce_loss, en_loss_cap, en_loss_img
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        self.InfoNCE_criterion = InfoNCE(tau=0.05)
        self.circle_loss = CircleLoss()
        self.TRL = TRL_SNH(num_classes=opt.proj_dim)
    ...
    def train(
        ...
        if torch.isnan(img_embs).any():
            raise RuntimeError("NaN detected in image embeddings during training.")
        cap_lens_tmp = cap_lens.copy()
        fixed_L = 36
        img_logits = self.img_pro_head(img_embs.view(img_embs.size(0), -1))
        if max(lengths) > fixed_L:
            cap_embs_pooled = F.interpolate(cap_embs.transpose(1, 2), size=fixed_L, mode='linear',
        ...
        if mode == 'ulb_train' or mode == 'hard':
            img_embs_lb, cap_embs_lb, cap_lens_lb = self.forward_emb(opt, images_ulb_train, captions_ulb_train,
                                                                     lengths_ulb_train)
            # --------------------------------
            #             img_embs_norm = F.normalize(img_embs, dim=1)
            #             img_embs_lb_norm = F.normalize(img_embs_lb, dim=1)
            #             sim_matrix = torch.matmul(img_embs_norm, img_embs_lb_norm.T)
            # -----------------------------------
            ...
            if True:
                ...
                for i in range(len(ids)):
                    ...
                    # pseudo_y = torch.argmax(pseudo_label[i]).item()
                    # cg_counter[ids[i]][pseudo_y] += 1
                # for i in range(len(ids_ulb_train)):
                #     distri_bank[ids_ulb_train[i]] = pseudo_label_lb[i].detach().cpu().numpy()
                    # pseudo_y = torch.argmax(pseudo_label_lb[i]).item()
                    # cg_counter[ids[i]][pseudo_y] += 1
            ...
            for i in range(max_idx.size(0)):
                ...
                cos_sim = nn.functional.cosine_similarity(current_p.view(1, -1),
                                                          pseudo_label_lb.view(pseudo_label_lb.size(0), -1), dim=-1)

                max_sim_idx = torch.argmax(cos_sim)

                # -----------------
                #                 sim_row = sim_matrix[i]
                #                 max_sim_idx = torch.argmax(sim_row)
                # -----------------
                ...
                if max(lengths_ulb_train) > fixed_L:
                    cap_embs_pooled = F.interpolate(max_sim_cap_embs_lb.unsqueeze(0).transpose(1, 2), size=fixed_L,
        ...
        if mode == 'warmup' or mode == 'warmup_sele':
            triplet_loss = self.InfoNCE_criterion(sims)
            ...
            # triplet_loss = self.InfoNCE_criterion(sims)
            # triplet_loss = self.circle_loss(sims)
            triplet_loss = self.criterion(
                opt,
                sims,
                hard_negative=hard_negative,
        ...
        if mode == 'lb_train' or mode == 'warmup' or mode == 'warmup_sele' or mode == 'hard':
            ...
            if mode == 'warmup' or mode == 'warmup_sele':
                for i in range(len(ids)):
                    ...
                    pseudo_y = torch.argmax(pseudo_label_img[i]).item()
                    cg_counter[ids[i]][pseudo_y] += 1
            # for i in range(len(ids)):
            #     pseudo_y = torch.argmax(pseudo_label_img[i]).item()
            #     cg_counter[ids[i]][pseudo_y] += 1
            ...
            if mode == 'lb_train' or mode == 'warmup' or mode == 'warmup_sele':
                ce_loss_img, en_loss_cap, en_loss_img = self.criterion_proj(img_logits, pseudo_label_cap,
                                                                            use_hard_labels=True, reduction='mean')
                ce_loss_cap, _, _ = self.criterion_proj(txt_logits, pseudo_label_img, use_hard_labels=True,
                                                        reduction='mean')
            elif mode == 'hard':
                ce_loss_img, en_loss_cap, en_loss_img = self.gce_loss(img_logits, pseudo_label_cap,
                                                                      use_hard_labels=True, reduction='mean')
                ce_loss_cap, _, _ = self.gce_loss(txt_logits, pseudo_label_img, use_hard_labels=True,
                                                  reduction='mean')
                # triplet_loss = self.TRL(sims, en_loss_img)
            if mode == 'lb_train':
        ...
        if mode == "eval_loss":
            ...
            for i in range(len(ids)):
                pseudo_y = torch.argmax(pseudo_label_img[i]).item()
                cg_counter[ids[i]][pseudo_y] += 1
            return triplet_loss, torch.softmax(img_logits, dim=-1), torch.softmax(txt_logits, dim=-1)
        ...
        elif mode == 'warmup' or mode == 'warmup_sele':
            return triplet_loss.item(), ce_loss_img.item(), ce_loss_cap.item(), -en_loss_img.item(), -en_loss_cap.item()
        elif mode == 'hard':
            return triplet_loss.item(), ce_loss_img.item()
        else:  # 'ulb_train'
...
class InfoNCE(nn.Module):
    ...
    def forward(self, scores):
        """
        Args:
            scores: Tensor of shape [B, B], pairwise image-text similarity matrix.
        Returns:
            Symmetric InfoNCE loss.
        """
        B = scores.size(0)
        labels = torch.arange(B).to(scores.device)
        loss_i2t = F.cross_entropy(scores / self.tau, labels)
        loss_t2i = F.cross_entropy(scores.T / self.tau, labels)
...
class CircleLoss(nn.Module):
    ...
    def forward(self, scores):
        ...
        # Mask diagonal to build negative pairs.
        neg_scores = scores + torch.eye(B).to(scores.device) * (-1e12)
        neg_scores = neg_scores.view(B, -1)  # [B, B]

        # Circle loss weighting terms.
        ap = torch.clamp_min(1 + self.m - pos_scores, min=0.0)
        an = torch.clamp_min(neg_scores + self.m, min=0.0)

        # Exponent offsets.
        delta_p = 1 - self.m
        delta_n = self.m

        logit_p = -self.gamma * ap * (pos_scores - delta_p)
        logit_n = self.gamma * an * (neg_scores - delta_n)

        loss_p = F.softplus(torch.logsumexp(logit_p, dim=0))
        loss_n = F.softplus(torch.logsumexp(logit_n, dim=1))

        return (loss_p.mean() + loss_n.mean()) / 2


class TRL_SNH(nn.Module):
    def __init__(self, delta_gamma=0.1, margin_base=0.5, num_classes=2):
        super(TRL_SNH, self).__init__()
        self.delta_gamma = delta_gamma  # Delta for dynamic margin adjustment
        self.margin_base = margin_base  # Base margin (reserved)
        self.max_entropy = math.log(num_classes)  # Used to normalize entropy
        self.eps = 1e-6

    def forward(self, sim_matrix, en_loss_img):
        """
        Args:
            sim_matrix: Tensor [B, B], similarity matrix S(Ii, Tj).
            en_loss_img: Tensor [B], entropy per sample, expected in [0, log(K)].
        Returns:
            Scalar TRL loss for hard negatives.
        """
        B = sim_matrix.size(0)
        device = sim_matrix.device

        # Step 1: Normalize entropy to uncertainty in [0, 1].
        uncertainty = en_loss_img.clamp(min=self.eps) / self.max_entropy
        uncertainty = uncertainty.clamp(self.eps, 1.0 - self.eps)

        # Step 2: Positive pair similarities (diagonal).
        pos_scores = sim_matrix.diag()  # [B]

        # Step 3: Hardest negatives (exclude diagonal).
        mask = torch.eye(B, device=device).bool()
        neg_scores = sim_matrix.masked_fill(mask, float('-inf'))  # [B, B]
        hardest_neg_t = neg_scores.max(dim=1)[0]  # S(Ii, Tj), j != i
        hardest_neg_i = neg_scores.max(dim=0)[0]  # S(Ij, Ti), j != i

        # Step 4: Dynamic soft margin.
        u = uncertainty
        gamma_i = 1 / (1 + u / (1 - u)) - self.delta_gamma  # [B]

        # Step 5: Two triplet-style terms.
        loss_i2t = F.relu(gamma_i - pos_scores + hardest_neg_t)
        loss_t2i = F.relu(gamma_i - pos_scores + hardest_neg_i)

        loss = (loss_i2t + loss_t2i).mean() / 2
        return loss
